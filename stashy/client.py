...
from .helpers import Nested, add_json_headers,ResourceBase,IterableResource
...
class PullRequest(ResourceBase,IterableResource):
    @response_or_error
    def get(self):
        """
...
class Dashboard(ResourceBase):
    pull_request=Nested(PullRequest,relative_path='pull-requests')
...
class Stash(object):
    ...
    ssh = Nested(Keys)
    repos = Nested(Repos)
    dashboard=Nested(Dashboard) #/ pull - requests
